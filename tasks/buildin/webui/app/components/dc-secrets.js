  ...
  render() {
    ...
    return html`
      ...
      <div class="card" style="margin-bottom:var(--dicode-space-md)">
        ...
        <div style="display:flex;gap:var(--dicode-space-sm);flex-wrap:wrap">
      ...
      ${this._status ? html`<p style="color:red;margin-bottom:var(--dicode-space-md)">${this._status}</p>` : ''}
      <table>
        <thead><tr><th>Key</th><th></th></tr></thead>
        <tbody>
          ${secrets.length === 0 ? html`
            <tr><td colspan="2" style="text-align:center;color:var(--dicode-muted)">No secrets stored.</td></tr>
          ` : secrets.map(k => html`
            <tr>
              <td><code>${k}</code></td>
              <td style="text-align:right">
                <button class="btn btn-sm" style="background:var(--dicode-red)" @click=${() => this._delete(k)}>Delete</button>
              </td>
            </tr>`)}
        </tbody>
      </table>`;

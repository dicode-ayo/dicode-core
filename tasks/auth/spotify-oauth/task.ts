...
import { Spotify as provider } from "../_oauth/providers.ts";
import { resolveClientId } from "../_oauth/builtin.ts";

const EXPIRY_KV_KEY = "spotify_oauth_expires_at";

export default async function main({ params, input, output, kv, dicode }: DicodeSdk) {
  const clientId        = resolveClientId("spotify", "SPOTIFY_CLIENT_ID");
  const baseURL         = (Deno.env.get("DICODE_BASE_URL") ?? "http://localhost:8080").replace(/\/$/, "");
  const existingToken   = Deno.env.get("SPOTIFY_ACCESS_TOKEN");
  const existingRefresh = Deno.env.get("SPOTIFY_REFRESH_TOKEN");
  const redirectURI     = `${baseURL}/hooks/spotify-oauth`;
  const scope           = await params.get("scope") ?? "user-read-private user-read-email";
  const inp             = input as Record<string, unknown> | null;
  const code            = inp?.code as string | undefined;

  if (code) {
    const verifier = await kv.get("spotify_oauth_verifier") as string | null;
    if (!verifier) return output.html(`<p style="color:red">PKCE verifier missing — click Run to restart.</p>`);
    await kv.delete("spotify_oauth_verifier");
    const tokens = await exchangeCodePKCE({ provider, clientId, redirectURI, code, verifier });
    await storeTokens(tokens, kv, dicode);
    const stored = ["SPOTIFY_ACCESS_TOKEN"];
    if (tokens.refresh_token) stored.push("SPOTIFY_REFRESH_TOKEN");
    return output.html(successHtml("Spotify", stored));
  }
  ...
  if (existingToken) {
    ...
    if (!tokenExpiresWithin(expiresAt, 60)) {
      ...
      return output.html(`<div style="font-family:system-ui,sans-serif;padding:2rem">
        <h2 style="color:#1a7f37">Spotify token still valid</h2>
    ...
    if (existingRefresh) {
      try {
        ...
        await storeTokens(tokens, kv, dicode);
        console.log("Spotify access token refreshed silently");
        if (inp !== null) return { refreshed: true };
        return output.html(successHtml("Spotify (refreshed)", ["SPOTIFY_ACCESS_TOKEN"]));
      } catch (err) {
  ...
  await kv.set("spotify_oauth_verifier", verifier);
  ...
  return handleAuthNeeded({
    name: "Spotify", authURL, redirectURI, scope, color: "#1DB954",
...
async function storeTokens(
  ...
  await dicode.secrets_set("SPOTIFY_ACCESS_TOKEN", tokens.access_token);
  if (tokens.refresh_token) await dicode.secrets_set("SPOTIFY_REFRESH_TOKEN", tokens.refresh_token);

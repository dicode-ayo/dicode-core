...
// CERTIFICATE block. Guards relay.ca_file against a present-but-empty or
// non-PEM file that would otherwise export an unusable CA.
func pemContainsCertificate(pemBytes []byte) bool {
...
// isPathUnderDir reports whether path resolves inside dir. Used to tell a
// relay.ca_file that the in-process relay-server will generate under the data
// dir (tolerate absence) from an operator-provided external path (fail fast).
...
//   - buildin/resume-state-cleanup: retention_seconds default, from
...
func applyBuiltinOverrides(k task.Kinded, cfg *config.Config) task.Kinded {
	...
	// Without this the task.yaml-baked default silently ignores whatever an
	// operator configures, exactly as run-inputs-cleanup's override above
	// exists to prevent.
...
func buildWebUI(ctx context.Context, cfg *config.Config, configPath, version, dataDir string, database db.DB, reg *registry.Registry, eng *trigger.Engine, localSecrets secrets.Manager, rec *registry.Reconciler, sourceMgr *webui.SourceManager, gateway *ipc.Gateway, logBroadcaster *webui.LogBroadcaster, managedRuntimes []pkgruntime.ManagedRuntime, approvalGate *approval.Gate, replayer *registry.Replayer, log *zap.Logger) (*webui.Server, error) {
	...
	if relayConfigured(cfg) {
		...
		// OAuth/webhook listener. When server_url carries an explicit non-default
		// port and broker_url is unset, ResolvedBrokerURL derives an OAuth base
		// URL on the *mTLS* port, which is wrong. Warn so the operator sets
		...
		if cfg.Relay.CAFile != "" {
			...
			switch {
			case err == nil:
				// A present-but-empty/garbage file would export an unusable CA
				// and silently drop the relay to WebPKI verification (which
				// rejects a self-signed broker) — fail fast with a clear message.
				if !pemContainsCertificate(caPEM) {
					return nil, fmt.Errorf("relay.ca_file %q does not contain a PEM CERTIFICATE block", cfg.Relay.CAFile)
				}
				if err := os.Setenv("DICODE_RELAY_CA_PEM", string(caPEM)); err != nil {
					return nil, fmt.Errorf("setenv DICODE_RELAY_CA_PEM: %w", err)
				}
			case os.IsNotExist(err) && isPathUnderDir(cfg.Relay.CAFile, dataDir):
				// A ca_file under the data dir is the in-process relay-server's
				// self-signed cert, generated by relay-server-body after this
				// boot path. The relay-client self-heals by reading that known
				// path at each connect attempt (see relay-client/task.ts), so a
				// missing file here is expected on first run — warn, don't fail.
				log.Warn(
					"relay.ca_file does not exist yet — the in-process relay-server will " +
						"generate it and the relay client picks it up on its next connect " +
						"attempt (no restart needed): " + cfg.Relay.CAFile,
				)
			default:
				// A missing ca_file outside the data dir is an operator-provided
				// path for an external broker — a typo or unprovisioned cert.
				// Fail fast rather than silently running with a dead tunnel.

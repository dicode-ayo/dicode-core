...
// TestEnforcement_Env_RelayImportNeedsWildcard reproduces dicode-core#412: the
// buildin/relay-server-body task does `import "npm:dicode-relay/start"` (Deno
// node-compat), whose transitive deps read process.env at module init beyond
// any small declared set. Under the restrictive --allow-env built from declared
// names the import throws NotCapable; with env: ["*"] (bare --allow-env) it
// imports cleanly while named entries still forward their host values.
//
// Skipped in -short / when Deno or the npm package can't be fetched.
func TestEnforcement_Env_RelayImportNeedsWildcard(t *testing.T) {
	if testing.Short() {
...
export default async function main() {
  return typeof startServer + ":" + (Deno.env.get("DICODE_DATADIR") ?? "MISSING");
	...
	// Fix: env: ["*"] + named entries → import succeeds AND DICODE_DATADIR is
	// forwarded (mirrors tasks/buildin/relay-server-body/task.yaml).
	wildcard := &task.Spec{
		ID: "relay-wildcard", Name: "relay-wildcard", Runtime: task.RuntimeDeno,
		Trigger: task.TriggerConfig{Manual: true}, Timeout: 120 * time.Second,
		Permissions: task.Permissions{
			Net: []string{"*"},
			Env: []task.EnvEntry{{Name: "*"}, {Name: "DICODE_DATADIR"}, {Name: "DICODE_VERSION"}},
		},
	}
	r = e.runSpec(t, script, wildcard)
	if r.Error != nil {
		t.Fatalf("relay import still failed under env: [\"*\"]: %v\nlogs:\n%s", r.Error, logsFor("relay-wildcard"))
	}

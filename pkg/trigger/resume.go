...
func (e *Engine) suspendRun(spec *task.Spec, opts *pkgruntime.RunOptions, result *pkgruntime.RunResult) (bool, error) {
	...
	// resumeStateThresholdBytes, the state is durably written to the storage
	// task FIRST and only its {store,key} handle is persisted on the runs
	// row — never the reverse order. If the blob write fails, suspendRun
	// returns an error without ever calling SuspendRun, so the run fails
	// loudly (dispatch's caller marks it StatusFailure) instead of landing a
	// suspended row with a reference to nothing.
	...
	suspended, err := e.registry.SuspendRun(context.Background(), opts.RunID,
		state, result.ResumeSchema, token, nowMs, deadlineMs, carryJSON, redactedFields, blobRef)

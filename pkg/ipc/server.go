...
type Server struct {
	...
	resumeStateStore resumeStateBlobStore // optional; enables dicode.runs.delete_resume_state blob deletion (#570)
...
// resumeStateBlobStore is the subset of *registry.ResumeStateStore the IPC
// server calls, narrowed the same way inputBlobStore is: it lets tests stub a
// store whose Delete fails without a real storage-task round trip.
type resumeStateBlobStore interface {
	Delete(ctx context.Context, key string) error
}

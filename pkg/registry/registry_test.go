...
func TestSuspendRun_RoundTrip(t *testing.T) {
	...
	if _, err := r.SuspendRun(ctx, runID, state, schema, token, suspendedAt, deadline, nil, nil, nil); err != nil {
		t.Fatal(err)
	}
...
func TestSuspendRun_NoDeadlineNilBlobs(t *testing.T) {
	...
	if _, err := r.SuspendRun(ctx, runID, nil, nil, "tok", time.Now().UnixMilli(), 0, nil, nil, nil); err != nil {
		t.Fatal(err)
	}
...
func TestCleanupStaleRuns_SkipsSuspended(t *testing.T) {
	...
	if _, err := r.SuspendRun(ctx, suspended, []byte(`{}`), nil, "tok", time.Now().UnixMilli(), 0, nil, nil, nil); err != nil {
		t.Fatal(err)
	}
...
func TestSuspendRun_DoesNotResurrectCancelled(t *testing.T) {
	...
	suspended, err := r.SuspendRun(ctx, runID, []byte(`{"step":1}`), nil, "tok", time.Now().UnixMilli(), 0, nil, nil, nil)
	if err != nil {

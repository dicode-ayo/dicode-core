...
func TestSuspendRun_WithBlobRef_WritesOffloadColumnsNotInline(t *testing.T) {
	...
	if _, err := r.SuspendRun(ctx, runID, []byte(`{"should":"not persist inline"}`), nil, "tok", time.Now().UnixMilli(), 0, nil, nil, ref); err != nil {
		t.Fatal(err)
	}
...
func TestSuspendRun_NilBlobRef_LeavesOffloadColumnsEmpty(t *testing.T) {
	...
	if _, err := r.SuspendRun(ctx, runID, state, nil, "tok", time.Now().UnixMilli(), 0, nil, nil, nil); err != nil {
		t.Fatal(err)
	}
...
func TestListExpiredResumeStates_ExcludesSuspendedRows(t *testing.T) {
	...
	mustSuspendWithBlob := func(id string, storedAt int64) {
		...
		if _, err := r.SuspendRun(ctx, id, nil, nil, "tok-"+id, time.Now().UnixMilli(), 0, nil, nil, ref); err != nil {
			t.Fatal(err)
		}
	}
...
func TestClearResumeStateBlob(t *testing.T) {
	...
	if _, err := r.SuspendRun(ctx, id, nil, nil, "tok", time.Now().UnixMilli(), 0, nil, nil, ref); err != nil {
		t.Fatal(err)
	}

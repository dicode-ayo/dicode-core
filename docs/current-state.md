...
### `pkg/webui/` ✅
- `server.go` — chi router, all REST + UI endpoints, HTMX templates embedded via `//go:embed`
- REST API endpoints including `POST /api/runs/{runID}/kill`
- Audit logs: run requested via API, kill requested via API
- Trigger editor: cron, webhook, manual, chain, **daemon** (with restart policy selector)
- `LogBroadcaster` — io.Writer, ring buffer (300 lines), SSE fan-out
- Session store, secrets manager UI, Monaco editor, AI chat, config code editor
- HTMX live log fix: `hx-select="#run-status-card"` prevents full-page nesting on poll

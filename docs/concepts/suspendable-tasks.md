...
- **`params` survive a resume; the original trigger input does not.** The
  ...
  The stored params carry-over is redacted the same way run inputs are: if a
  fire-time param's name is declared as a `secret:` key in the task's own
  `permissions.env` AND its value matches that live secrets-chain entry, the
  persisted `runs.resume_params` row substitutes the redaction placeholder for
  it and lists the dotted `params.<name>` path in
  ...
  chain (only if the task's *current* `permissions.env` still grants that
  name — otherwise the resume fails outright rather than guessing), not read
  back from the placeholder. A param whose name isn't among the task's own
  declared secret grants — however sensitive it *looks* (`token`, `key`,
  `password`, …) — is never resolved against the secrets chain and is left
  as-is in the stored blob: dicode has no other way to recover a literal
  value later, and a resumed run must never run with a value it never had or

...
2. The session is served by the agent task configured as `ai.create_task`. **Not yet sandbox-isolated**: `SandboxPath` is plumbed through the session struct/DB/IPC/REST layers but never actually assigned to a dev-mode clone — that wiring is still open (Phase 1, see the [design doc](../design/ai-task-authoring.md)'s disconnects table). Today an authoring session's edits land directly against the task's live source, not an isolated clone. `ai.create_task` defaults to `buildin/task-create` (`pkg/config/config.go`), a `kind: PipelineTask` shipped in `dicode-buildin/taskset.yaml`: the agent turn followed by the disk check described in step 5. Provider, model and skills live on its first stage, `buildin/task-create-turn` — an `ai-agent` override carrying the `dicode-task-dev` + `dicode-basics` skills and defaulting to OpenAI's `gpt-4o` (set `OPENAI_API_KEY`). Override that entry to swap provider, model, or skills; point `ai.create_task` at a task of your own to replace the whole thing, checks included.
...
5. **What the turn claims is checked against disk.** `buildin/task-create` is a `kind: PipelineTask`, not a bare agent task: stage 1 is the agent turn (`buildin/task-create-turn`), stage 2 is `buildin/verify-task-written`, which reads the target directory and fails when it holds no task the agent wrote — no `task.yaml`, no `kind: Task` in it, no runnable body, or nothing but the untouched scaffold. A failing stage fails the pipeline, so a turn that wrote nothing settles as a **failed run**, not a successful one, and the CLI exits non-zero. The reply is the agent's own account of its work and is never the evidence that the work happened; the verify stage passes it through so the caller still sees it. A task directory the session could not resolve arrives as the sentinel `unknown` and leaves the check unevaluated rather than failing the turn.

   The check lives in the pipeline rather than in the control plane because the control plane fires whatever `ai.create_task` names and has no business knowing what an authoring turn should leave behind. The trade is that it ships *with* this pipeline: point `ai.create_task` at a bare agent task and the turn goes back to being taken at its word. Note also that a pipeline stage cannot suspend, so an agent that pauses for clarification fails the pipeline — the shipped one-shot path never suspends, but a custom override that does needs its own pipeline, or none.
...
The dicode MCP surface itself (`dicode-buildin/mcp/task.ts`) only exposes `list_tasks`, `get_task`, `run_task`, `list_sources`, `switch_dev_mode`, and `test_task` — an MCP agent drives task authoring by calling the same `/api/task/*` authoring endpoints directly (with its API key), not through a dedicated MCP tool.

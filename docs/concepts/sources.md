...
The `buildin` namespace is the exception: the gate auto-approves it before any hash check, since it ships with the binary and is trusted by default. When `buildin` tracks a moving `branch` (the default), that auto-approve bypass is unconditional, same as ever — a moving branch is expected to move. Once `buildin` is pinned to a `tag`, though, the gate additionally tracks the git commit each approved record was observed at; a re-cut tag whose resolved commit no longer matches the one on record holds the task pending instead of silently re-arming it on new content, exactly like any other pinned source ([#832](https://github.com/dicode-ayo/dicode-core/issues/832)). An install upgrading onto this check does not have its existing `buildin` inventory pend: the first admit after upgrade (or after pinning) seeds the current commit as the baseline rather than treating "no commit on record yet" as drift.
